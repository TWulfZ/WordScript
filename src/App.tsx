...
export default function App() {
  ...
  const handleColumnChange = (name: string, e: React.FormEvent<HTMLInputElement>) => {
    ...
    setColumns((prevColumns) =>
      prevColumns.map((col) => (col.name === name ? { ...col, value: newValue } : col)),
    );
  ...
  return (
    <div className="h-full w-full">
      <Card className="h-full min-h-screen w-full">
        <CardHeader>
          <CardTitle className="flex items-center space-x-2 text-[4rem]">
            <img src="/wordscript-text.svg" alt="WordScript Logo" className="h-20"></img>
          </CardTitle>

          <CardDescription>
            Importa datos CSV de Google Forms y genera documentos de Word con plantillas
            personalizadas
          </CardDescription>
        </CardHeader>
                ...
                <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    ...
                    <Info title="Marcador de columna">
                      Este es el marcador que se utilizará en Word para buscar y reemplazar los
                      valores de cada columna donde se encuentre este texto. Por ejemplo:
